...
let potato = {
    ...
    y: screenHeight - 50, // Posizione verticale
...
function createKnife() {
    ...
    knives.push({
        x: screenWidth,
        y: Math.random() < 0.5 ? screenHeight - knifeHeight : screenHeight - knifeHeight - 50,
        width: 50,
        height: knifeHeight,
    });
}
...
document.addEventListener('keydown', (event) => {
    if (event.code === 'Space') {
        ...
        if (!potato.isDucking) {
            potato.y -= 50; // Salto
            setTimeout(() => { potato.y += 50; }, 300); // Torna giù dopo un po'
        } else {
